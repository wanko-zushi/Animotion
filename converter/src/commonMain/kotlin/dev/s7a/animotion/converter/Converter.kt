...
class Converter(private val resourcePack: ResourcePack) {
    ...
    fun save(destination: Path) {
        ...
        items.forEach { (key, value) ->
            ...
            val overrides = value.flatMap { (bbModel, parts) ->
                bbModel.textures.forEachIndexed { index, texture ->
                    texture.save(destination, resourcePack.animotion.settings.namespace, bbModel.name, index)
                }
        ...
        resourcePack.animotion.models.forEach { model ->
            val parts = model.outliner.map { outliner ->
                val (material, customModelData) = modelToItem[model.name to outliner.name] ?: throw RuntimeException("${model.name} & ${outliner.name} is not assigned to any item")
                if (outliner.origin.size != 3) throw RuntimeException("Invalid outliner origin length: (Model: ${model.name}, Outliner: ${outliner.name})")
                val position = Vector3(outliner.origin[0], outliner.origin[1], outliner.origin[2])
                val rotation = outliner.rotation?.let {
                    if (outliner.rotation.size != 3) throw RuntimeException("Invalid outliner rotation length: ${outliner.rotation.size} (Model: ${model.name}, Outliner: ${outliner.name})")
            ...
            val animations = model.animations.associate { animation ->
                ...
                val loopType = when (animation.loop) {
                ...
                val animators = animation.animators.entries.associate { (outlinerUuid, animator) ->
                    ...
                    if (animator.type != BlockbenchAnimatorType.Bone) throw RuntimeException("Unsupported animator type: ${animator.type}")
                    val frames = animator.keyframes.map { frame ->
                        if (frame.interpolation != Keyframes.Interpolation.Linear) throw RuntimeException("Unsupported interpolation: ${frame.interpolation}")
                        if (frame.dataPoints.size != 1) throw RuntimeException("Unsupported data_points length: ${frame.dataPoints.size} (Model: ${model.name}, Animation: ${animation.name}, Animator: ${animator.name})")
                        val frameType = when (frame.channel) {
